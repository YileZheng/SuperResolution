...
#     corrs_eachchar = np.zeros((7))
    ...
    for i, (XI,_, labels, ims, _) in enumerate(testloader):
        ...
        if use_gpu:
            x = Variable(torch.Tensor(XI).to(device))
            lbl = Variable(torch.LongTensor(YI).to(device))
        else:
...
#         print(y_pred.shape)
        ...
        for j in range(lbl.shape[0]):
            n_correct = 0
            sim_preds, chars_preds = decode(preds[j].data)
#             print(sim_preds,lbl[i].data)
            ...
            # if n_correct != 7:
            #     wrong_pred.append(chars_preds)   
            #     wrong_im.append(ims[j])

        # print(preds.shape)
        corrs_eachinst = np.append(corrs_eachinst,corr_eachinst)
        # n_correct = np.array(corr_eachinst)
        # wrong_ims = np.append(wrong_ims, wrong_im )
        # wrong_preds = np.append(wrong_preds, wrong_pred, axis=0)
        # print(corrs_eachinst, wrong_ims, wrong_preds)
        
        if i%100 ==1:
            # print(wrong_ims, wrong_pred)
    ...
    # f_log = open("log_{}.txt".format(perspect), 'a')
    # for i in range(len(wrong_ims)):
    #     f_log.write(str(wrong_ims[i]) + "|" + str(wrong_pred) + "\n")
    # f_log.close()
